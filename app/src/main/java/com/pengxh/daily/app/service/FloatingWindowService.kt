...
import com.pengxh.daily.app.utils.MessageChannelDispatcher
...
class FloatingWindowService : Service(), CoroutineScope by CoroutineScope(Dispatchers.Main) {
    ...
    private val channelDispatcher by lazy { MessageChannelDispatcher(this) }
    ...
    private fun sendChannelMessage() {
        channelDispatcher.send("内存使用预警", "当前内存使用已超过90%，请关注设备运行情况")
    }
